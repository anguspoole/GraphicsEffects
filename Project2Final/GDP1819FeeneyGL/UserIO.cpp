						  ...
						  int mods)
{
	...
	cMeshSceneObject* pSM = findObjectByFriendlyName("SM_Object");
...
//	std::cout << pTheOneLight->atten.x << ", " << pTheOneLight->atten.y << ", " << pTheOneLight->atten.z << std::endl;
	...
	if ( IsAltDown(window) )
	{
		//cMeshSceneObject* pTheRabbit = findObjectByFriendlyName("Roger");

		if ( glfwGetKey( window, GLFW_KEY_W ) )	{ g_pFlyCamera->eye.z += cameraMoveSpeed;	}
		if ( glfwGetKey( window, GLFW_KEY_S ) )	{ g_pFlyCamera->eye.z -= cameraMoveSpeed;	}
		if ( glfwGetKey( window, GLFW_KEY_A ) )	{ g_pFlyCamera->eye.x -= cameraMoveSpeed;	}
		if ( glfwGetKey( window, GLFW_KEY_D ) ) { g_pFlyCamera->eye.x += cameraMoveSpeed;	}
		if ( glfwGetKey( window, GLFW_KEY_Q ) )	{ g_pFlyCamera->eye.y += cameraMoveSpeed;	}
		if ( glfwGetKey( window, GLFW_KEY_E ) )	{ g_pFlyCamera->eye.y -= cameraMoveSpeed;	}
