...
{
	// Get the MiG-29
	cMeshSceneObject* pMig29 = findObjectByFriendlyName("Mig-29");

	// So show that the points move with the mig, I'll update the position and orientation here	
	pMig29->adjMeshOrientationEulerAngles( glm::vec3( 0.5f, 1.0f, 0.75f ), true );
	...
	float forwardSpeedThisFrame = forwardSpeed * (float)deltaTime;
	...
	glm::vec4 leftWingTipContactPoint_ModelSpace = glm::vec4(0.3692f, 0.0f, -0.3f, 1.0f);
	glm::vec4 rightWingTipContactPoint_ModelSpace = glm::vec4(-0.3692f, 0.0f, -0.3f, 1.0f);
	...
	DrawObject( pDebugBall, matIdentity, shaderProgramID, g_pFBOMain);
