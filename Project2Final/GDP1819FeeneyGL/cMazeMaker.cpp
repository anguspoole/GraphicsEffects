...
void cMazeMaker::PrintMaze(std::vector<cMeshSceneObject*> &vec_pObjectsToDraw)
{
	for (unsigned int a = 0; a < this->maze.size(); a++) 
	{
		for (unsigned int b = 0; b < this->maze[a].size(); b++) 
		{
			if (this->maze[a][b][0]) 
			{
				...
				pCubeForBallsToBounceIn->setDiffuseColour(glm::vec3(0.0f, 0.0f, 0.0f));
				pCubeForBallsToBounceIn->bDontLight = false;
				pCubeForBallsToBounceIn->friendlyName = "cube" + std::to_string(a) + std::to_string(b);
				pCubeForBallsToBounceIn->meshName = "cube_flat_shaded_xyz_n_uv.ply";		// "cube_flat_shaded_xyz.ply";
				pCubeForBallsToBounceIn->setUniformScale(1.0f);
				...
				pCubeForBallsToBounceIn->position = glm::vec3(a * 2.001f, 0.0f, b * 2.001f);
				...
				pCubeForBallsToBounceIn->vecTextures.push_back(sTextureInfo(testObjectTexture));
